...
#remark[
    Заметим также, что $UU$ и $VV$ обладат свойством однородности: если $VV_f$ --- минимльная диагонально вогнутая функция $f$, то $VV_(epsilon f(x/epsilon)) = VV_f, space epsilon>0$. Аналогично для $UU$.
]
#remark[
    Пусть отрезок $E$ --- граница между двумя областями, на которых функция $f$ гладкая, причем в $E$ она имеет разрыв первой производной. Тогда диагональная вогнутость на этой границе эквивалентна $D$-вогнутости.
]
#proof[
    Действительно, легко понять что вогнутость по направлению в точке из отрезка будет достигаться при неравенстве на правую и левую производные. Заметим, что формула производной по направлению $(beta,1)$ линейна по $beta$. Тогда для $D$-вогнутости достаточно рассмотреть вогнутость лишь по крайним значениям $beta$, что эквивалентно диагональной вогнутости.
]
...
#proof[
    ...
    $
    Таким образом, доказали вогнутость внутри ромба, вогнутость в полосах $Sigma_R [v_1,omega], space Sigma_L [omega,v_2]$ также следует из условия. Вогнутость на границе ромба следует из замечания 2.
]
...
Определим $Sigma_(h r b) [a_0,b_0]= Sigma \\ (Sigma_L [-oo,a_0] union Sigma_R [b_0,oo])$ и функцию $V_(h r b)$ в этой области:
$
...
#proof[
    Эта теорема является переформулировкой теоремы (4.1.2) из [1]. Пользуясь рассуждениями из [2] о соответствии между функциями в параболе и полосе, получаем что существует функция $V^epsilon$, структурно соответствующая условию теоремы. По теореме 10 она будет минимальной диагонально вогнутой, то есть $V^epsilon = VV^epsilon$. 
]
...
#proof[ (Теоремы 1)  #h(1cm)
    ...
    Выпишем оценку:
    $
    2m_R^prime (b_0) = m_R^prime (b_0) + m_L^prime (a_0) = f^prime (b_0) - f^prime (a_0) + m_L (a_0)-m_R (b_0) > (b_0-a_0)f^(prime prime) (b_0) >=0.
    $
